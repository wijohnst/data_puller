...
function writeUntrackedToSheet(){
  ...
  try{
    // agents = getColByHeading(targetSheet, searchParam).flat().filter(agent => agent !== searchParam && agent !== "" );
...
function writeReportToSheet(targetRoutes, dataTarget){
  ...
  targetRoutes.map(({ sheetName, columnName }) => {
    ...
    try{
      agents = getColByHeading(sheetName, agentSearchParam).flat().filter(agent => agent !== agentSearchParam && agent !== "" );
    }
    catch(err){
    ...
    if(headerIndex > 0){ //If there is a column in Summary Report matching ${columnName}

      const targetColNum = headerIndex + 1;
    ...
    }else{ //If a column in Summary Report cannot be found that matches ${columnName}
      throwAlert(`The system cannot find a column in Summary Report that matches ${columnName}. Please make sure the column headers in Summary Report are spelled correctly. To update this code see reportFormatter.gs >> writeReportToSheet()`);
      console.log(`Column indexing error. Cannot find a column header in Summary Report that matches ${columnName}. See reportFormater.gs >> writeReportToSheet()`)
    }
  })
}
